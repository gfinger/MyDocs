# Handbuch_v7 zur Forschungstabelle Lorsch

**Datenbasis:** `lorsch-forschung_gesamttabelle_102-135_arbeitsstand_v6-1.md`

**Version:** v7 – zweistufiges Modell fortgeführt; zusätzliche Normierungsentscheidung zu `Homilien`/`Homiliensammlung`.
...
## Vorläufige Normierungsentscheidung v6: `Liturgisches Buch`, `Liturgik` und `Lehrschrift`

Diese Entscheidung ist vorläufig festgehalten und noch nicht in der Forschungstabelle umgesetzt.

1. `Liturgisches Buch` bleibt eine Buchgattung, aber nur für Bücher bzw. Textbestände, die primär dem Vollzug der Liturgie dienen, also liturgische Gebrauchsbücher, Formulare, Gebete, Ordnungen oder Riten enthalten. Beispiele können Sakramentar, Messbuch/Missale, Pontifikale, Benedictionale, Ordo oder Litanei sein. Ob solche Spezialformen künftig selbst Buchgattungen bleiben oder als `Sub-Gattung` unter `Liturgisches Buch` geführt werden, bleibt ein eigener Redaktionsschritt.
...
3. Für Texte über Liturgie gilt vorläufig: `Buchgattung = Lehrschrift` oder `Traktat`, `Sub-Gattung = Liturgik`, `Thema = Liturgie`. Bei engerem Gegenstand kann die Sub-Gattung präziser lauten, z. B. `Messerklärung` oder `Eucharistietheologie`.
4. `Lehrschrift` bezeichnet die didaktische Form eines Textes. Die fachliche Differenzierung soll über `Sub-Gattung` und `Thema` erfolgen. Dadurch werden mathematische Lehrschriften wie Boethius, *De institutione arithmetica*, von theologischen Lehrtexten getrennt, ohne neue Buchgattungen zu vermehren.
5. Vorläufige Sub-Gattungen unter `Lehrschrift` können sein: `Arithmetik`, `Grammatik`, `Dialektik`, `Computus`, `Liturgik`, `Theologische Lehrschrift`, `Eucharistietheologie`.
6. `Theologie` wird nicht als eigene Buchgattung eingeführt. Theologische Inhalte werden über `Sub-Gattung`, `Thema` und gegebenenfalls `Überlieferungslinie` erfasst.

Beispiele:

| Texttyp | Buchgattung | Sub-Gattung | Thema |
| --- | --- | --- | --- |
| Boethius, *De institutione arithmetica* | `Lehrschrift` | `Arithmetik` | `Arithmetik` |
| erklärender Text über liturgische Vollzüge | `Lehrschrift` oder `Traktat` | `Liturgik` | `Liturgie` |
| Messerklärung | `Traktat` oder `Lehrschrift` | `Messerklärung` | `Liturgie / Messe` |
| eucharistietheologischer Text | `Traktat` oder `Lehrschrift` | `Eucharistietheologie` | `Eucharistie` |
| liturgisches Gebrauchsbuch | `Liturgisches Buch` | noch zu entscheiden, z. B. `Sakramentar`, `Ordo`, `Litanei` | `Liturgie` |
...
- `Liturgisches Buch` und `Liturgik` trennen: `Liturgisches Buch` bezeichnet liturgische Gebrauchsbücher; `Liturgik` bezeichnet erklärende/lehrhafte Texte über Liturgie und soll künftig als `Sub-Gattung` unter `Lehrschrift` oder `Traktat` erscheinen.
- `Lehrschrift` als formale Oberkategorie verwenden; fachliche Differenzierung über `Sub-Gattung`, z. B. `Arithmetik` für Boethius, *De institutione arithmetica*, und `Theologische Lehrschrift`, `Liturgik` oder `Eucharistietheologie` für theologische Lehrtexte.
