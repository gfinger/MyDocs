# Handbuch_v8 zur Forschungstabelle Lorsch

**Datenbasis:** `lorsch-forschung_gesamttabelle_102-135_arbeitsstand_v6-1.md`

**Version:** v8 – Zwischenstand zur Reduktion der Buchgattungen: Einführung von `Fachtext` als vorläufiger Großgattung für fachbezogene Sachtexte.
...
## Vorläufige Normierungsentscheidung v6: `Liturgisches Buch` und `Liturgik`

Diese Entscheidung ist vorläufig festgehalten und noch nicht in der Forschungstabelle umgesetzt. Die frühere Lehrschrift-Regel aus v6 wird durch den Zwischenstand v8 zu `Fachtext` ersetzt; die Unterscheidung zwischen `Liturgisches Buch` und `Liturgik` bleibt bestehen.
...
- `Liturgisches Buch` und `Liturgik` trennen: `Liturgisches Buch` bezeichnet liturgische Gebrauchsbücher; `Liturgik` bezeichnet erklärende/lehrhafte Texte über Liturgie und soll künftig als `Sub-Gattung` unter `Fachtext` erscheinen.
